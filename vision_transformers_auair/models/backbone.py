...
from vision_transformers_auair.models.layers import DropPath, to_2tuple, trunc_normal_
...
def interpolate_pos_embed(pos_embed, target_grid_size):
    import torch.nn.functional as F

    B, N, D = pos_embed.shape
    total_tokens = N
    num_grid_tokens = (int((N - 25) ** 0.5)) ** 2  # 4225
    num_extra_tokens = N - num_grid_tokens  # 25

    print(f"Total tokens: {N}, Grid: {num_grid_tokens}, Extra: {num_extra_tokens}")

    extra_tokens = pos_embed[:, :num_extra_tokens]  # e.g. [CLS] + [DET]
    pos_tokens = pos_embed[:, num_extra_tokens:]  # patch tokens

    grid_size = int(num_grid_tokens**0.5)

    pos_tokens = pos_tokens.reshape(1, grid_size, grid_size, D).permute(0, 3, 1, 2)
    ...
    pos_tokens = pos_tokens.permute(0, 2, 3, 1).reshape(
        1, target_grid_size * target_grid_size, D
    )
...
def tiny(pretrained=None, **kwargs):
    ...
    if pretrained:
        ...
        expected = model.pos_embed.shape[1]
        state_dict["pos_embed"] = pos_embed[:, :expected, :]

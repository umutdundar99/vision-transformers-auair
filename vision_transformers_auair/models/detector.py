...
from vision_transformers_auair.models.backbone import *
from vision_transformers_auair.models.matcher import build_matcher
from vision_transformers_auair.utils import box_ops
from vision_transformers_auair.utils.misc import (
    NestedTensor,
    accuracy,
    get_world_size,
    interpolate,
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        assert len(out_logits) == len(
            target_sizes
        ), "Number of images in batch and target_sizes must be the same"
        assert (
            target_sizes.shape[1] == 2
        ), "target_sizes must contain the size of each image in the batch"
